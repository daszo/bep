...
\newcommand{\theSupervisor}{dr. Y. Tang} % Dr. Ing. L. Dorst
...
In this section an overview of the current state of information retrieval is given. Here there will be explained how IR moved from exact word matching to searching using meaning. 

\section{Traditional Information Retrieval}

In this section the techinques that were very domiant and are sometimes still used to day. THese techinques are characterized by their heaving empasis on lexical matching. What unites these techniques is the use of tokenization and normalization. Tokenization is the act of splitting up text into a set unit. A common way to tokenize it to break down a teks into words.
Part of Normalization is making all words lowercase. Stemming is also part of this pricess and involves casting words to their simplest for i.e. running to run and stop words removal.

\subsection{Inverse Index}

Inverted Index \cite{strathprints2621} is a method that creates a dictionary that saves for every term the documents it is part of. To search for a document, the tokens from the query where input into this dictionary and the intersection of all the documents that contained the tokens was retrieved. 

\subsection{BM25}
BM25 is a sparse retrieval ranking function that is the industry standard baseline for retrieval tasks. d works well on precise word extraction [source]. The order of words is not important as it uses the bag of words theorem. 
...
This method is made up of 3 parts. THe first part is the term frequency, $f(q_i, D)$. This is the amount of times that that a keyword $q_i$ occurs in the given document.

The second parts is the document lenght normalization, $\frac{|D|}{\text{avgdl}}$. This function of this is penalizing large documents, which otherwise would be dominant.

The last part is the inverted document frequency (IDF) \ref{eq:IDF}. This acts as a weight that rewards rarity by calculating how much it occurs in the entire corpus. 
\begin{equation}
\label{eq:IDF}
...
The variable k controls how important the occurence of new words are. Higher term frequency might not have to weight much more then a lower one.

The variable b controls the amount of lenght normalization

The variable k controls how important the occurence of new words are. Higher term frequency might not have to weight much more then a lower one.

The variable b controls the amount of lenght normalization.
% TODO: explain the variables better

\section{Transformer architecture}

In this section the transformer method is introduced [cite],. This is a method that utilizes position of words and derives meaning form this. The methods discussed in the previous section both suffer form the sematic gap. Which is a problem where the query uses a differnt lexical form to the documents itself. Tranfomers, are a deep neural neworks and cast words into ins latent space, which solves a large part of this problem. The latent space is a term used in deep learning and refers to the hidden parameters tha reside inside of the model and is of a different form then text. Transformer architecture excels at sequence to sequence tasks, which are tasks where the input and the output are sequences. [bert]

These there two types of NLP tasks, holistic and tokenized ones [bert]. The holistic type operates owith text on the sentence level. The tokenized answers question on the attribution entities. 
...
In the previous sections the methods described are what is sometimes called retrieve-then-rank strategiesOn the other hand, generative retrieval integrates the indexing and retrieval into one model. This allows for end to end optimization as the whole task is one differentiable model. It's output is the document identifier (docid) and learns the connection between the documents and the docids.
...
SE-DSI stands for sematic-enhanced differentiable search index and improves over the DSI method by including rehearsal steps and preplacing the unique indentifier with a unique strings derived from the contents of the documet.
