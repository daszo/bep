...
\textbf{Performance baseline BM25:} BM25 performs well on all metrics, but especially the No Thread dataset. The latter is not in line with  findings in \textcite{10.1145/3580305.3599903} and \textcite{NEURIPS2022_892840a6}. The reason for this might be because the No Thread dataset does not include the emails history, making the emails small, which is when BM25 performs best. Comparing 10,000 thread and 100,000 thread the performance drops a little which is similar to \textcite{10.1145/3580305.3599903} and \textcite{NEURIPS2022_892840a6}.
...
Also, when cleaning the 10k no thread dataset, less emails were bigger then 93.000

\textbf{Thread settings comparison:} There is a slight better performance between both BM25 and DSI on no thread then on thread. This could be explained by Thread containing a lot of duplicate email, confusing the algorithm. When inspecting the ratios thread performed a bit better, which could be because the training data contained more words, thus providing more data for the DSI model to learn from.
...
In all but the thread same mid, the doc to query generation was done using the subject and body. This was done my including "subject:" or "body:" respectively. This cluttered the queries as the model found that always very important. In further work this could be used 
