...
A perpetual problem of searching through emails is the sematic mismatch problem. THis causes a lot of searching time when users search through their emails. Currently the most used searching method is bm25[reference bm25]. ------ To circumvent this issue normalization, toke ionization, stop word removal and stemming is used. This however is not sufficient as it still needs a query to be quite similar to the context of the documents. It would be better if uses could ask question and the meaning of such question would be used for searching. Do this, dense vector retrieval [reference] could be used. ---- This turns text into a one dimensional vector and calculates the similarities between the query and the documents. This works better, but has the problem not being end to end differential. To solve this the [tay et all] proposed a generative method called the differentable search index (DSI) turning indexing and task training into one optimizable step. 

[SE-DSI, why because output likely to be similar and used in this research]
Generative retrieval (GR) is still a novel field and DSI has been mainly used for curated datasets or books. It is interesting to explore what DSI could be used for. To see how well DSI performs on an email dataset, the Enron corpus could be used [Enron reference]. In this research, we explore to what extent DSI can be used to search on the Enron cropus and how well does it compare to BM25. 
...
For years search relied on exact word matching. BM25 work wel on precise word extraction. To increase the likelyhood of word mathing normalization, tokenization, stop word removal and stemming is used. This however is not sufficient as it still needs a query to be quite similar to the context of the documents. This is often referred to as the sematic gap and is the focus of a lot of active research. 

The first breakthrough were transformer models [reference transformers]. This is a version of a recurrent neural network which could take sentences and attributes weights to the position of the words called attention. This attention allowed search on meaning and not exact word matching. This technique resulted in the creation of large language models (LLM) [reference LLM]. These were highly specialized in question and answering, which is very useful for retrieval as it allowed users to specify their question in the users natural language [to intoduction]. The big problem with LLM's is that they hallucinate and are not faitfull to sources. Dense vector retrieval solves this issue because it turns text into a one dimensional vector and calculates the similarities between the query and the documents. Although Dense vector retrieval can also use the transformer architecture it stays faitfull to the sources, by only retrieving the relevant doucments and not augmenting them. 
...
The limitations of this research is because we use MLE the task is a dichotomy, which takes relevance as binary which might not be representative of real world data contained in the Enron dataset.
